...
async def start(chat: TGChat) -> TGResponseMsg:
    msg_txt = (
        f'Hey { chat.first_name }! Nice to see you here. '
        'What\'s the passphrase?'
    )

    await cache_svc.set_conversation_status(
        chat.id,
...
async def on_reauthentication_required(chat_id: int) -> TGResponseMsg:
...
async def on_user_passphrase(msg: TGMessage) -> TGResponseMsg:
    await cache_svc.set_conversation_status(
        msg.chat.id,
        ConversationStatus.STANDBY)

    if auth_svc.checkpw(msg.text):
        await cache_svc.add_tg_authorized_chat(msg.chat.id)
